...
const c8* MenuAction_About(u8 op, i8 value)
{
    ...
    if (op == MENU_ACTION_SET)
    {
        ...
        {
            //UpdateIntroMusic();
            AKG_Decode();     // Process one frame of music playback.
...
static void PrintBigNum(u32 val)
{
    char buf[8];  // 7 digits + 1 for the null terminator.
    buf[7] = '\0';
    for (int i = 6; i >= 0; i--)
    {
        buf[i] = '0' + (val % 10);
...
{
    // Choose color based on power–up status.
    ...
    /*
       ...
       - If wall–walk mode is active, the ship is attached to an extreme wall (left or right),
         and it is climbing (g_PlayerY < SHIP_ROW), rotate the sprite.
         * On the left wall (g_PlayerX == 0): rotate 90° right so it faces right.
         * On the right wall (g_PlayerX == SCREEN1_COLS - 1): rotate 90° left so it faces left.
       - Otherwise, use the original upright sprite.
    */
    if (g_WallWalkActive &&
        (g_PlayerX == 0 || g_PlayerX == SCREEN1_COLS - 1) &&
        (g_PlayerY < SHIP_ROW))
    {
        if (g_PlayerX == 0)
        {
            // Left wall: rotate 90° to the right.
            SpriteFX_RotateRight8(g_SpritePatternShip, ship_buffer);
        }
        else // g_PlayerX == SCREEN1_COLS - 1 (right wall)
        {
            // Right wall: rotate 90° to the left.
            SpriteFX_RotateLeft8(g_SpritePatternShip, ship_buffer);
        }
    }
    else
    {
        // Use the normal (upright) ship sprite.
        Mem_Copy(g_SpritePatternShip, ship_buffer, sizeof(ship_buffer));
    }

    // Load the sprite pattern into VDP memory.
    VDP_LoadSpritePattern(ship_buffer, PATTERN_PLAYER, 1);

    /*
       Set the sprite position:
       - When climbing on an extreme wall (g_WallWalkActive true, attached to left or right, and g_PlayerY < SHIP_ROW),
         position using g_PlayerY.
       - Otherwise (when on ground or transitioning), force the ship onto ground level.
    */
    if (g_WallWalkActive &&
        (g_PlayerX == 0 || g_PlayerX == SCREEN1_COLS - 1) &&
        (g_PlayerY < SHIP_ROW))
...
static bool RunWave(void)
{
    ...
    while (g_Running)
    {
        ...
        if (g_WallWalkActive) {
            // NEW: Modified wall–walk movement with climbing and dropping behavior.
            if (g_PlayerX == SCREEN1_COLS - 1) { // On the right wall.
                if (go_right) {
                    // Climb upward on the right wall.
                    if (g_PlayerY > 0)
                        g_PlayerY--;
                }
                else if (go_left) {
                    // Drop down to ground and then move left.
                    if (g_PlayerY != SHIP_ROW)
                        g_PlayerY = SHIP_ROW;
                    if (g_PlayerX > 0)
                        g_PlayerX--;
                }
            }
            else if (g_PlayerX == 0) { // On the left wall.
                if (go_left) {
                    // Climb upward on the left wall.
                    if (g_PlayerY > 0)
                        g_PlayerY--;
                }
                else if (go_right) {
                    // Drop down to ground and then move right.
                    if (g_PlayerY != SHIP_ROW)
                        g_PlayerY = SHIP_ROW;
                ...
                // Not at an extreme: move horizontally.
            ...
            // Normal movement: horizontal at ground level.
            if (go_left && (g_PlayerX > 0))
                g_PlayerX--;
            else if (go_right && (g_PlayerX < SCREEN1_COLS - 1))
                g_PlayerX++;
        ...
        {
            if (g_WallWalkActive && (g_PlayerY < SHIP_ROW))
            {
                // When wall–walking AND climbing (i.e. not on ground), fire sideways.
                g_BulletX = g_PlayerX;
...
void main(void)
{
    ...
    // Enable VBlank interruption for timing synchronization
    VDP_EnableVBlank(TRUE);
    Bios_SetHookCallback(H_TIMI, VBlankHook); // VBlankHook() sets a flag every VBlank
    ...
    while (!g_Exit)
    {
        ...
        // Copy the music data from ROM (g_teste) to the playback address (0xD000)
        //Mem_Copy(g_teste, (void*)MUSIC_ADDRESS, MUSIC_SIZE);
        Pletter_UnpackToRAM(g_teste, (void*)MUSIC_ADDRESS);

        // Start music playback using the data now at MUSIC_ADDRESS.
        AKG_Init((const void*)MUSIC_ADDRESS, 0);

        Print_SetPosition(12, 20);
        PrintBigNum(MUSIC_SIZE);

        while (!g_Exit && !g_StartGame)
        {
            //WaitVBlank();  // Wait for the next VBlank.
        ...
        // Stop the music when ESC is pressed.
        AKG_Stop();
